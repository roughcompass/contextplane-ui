...
import type { AdminOperationDefinition } from "./adminOperations";
...
function safeFailure(error: unknown): {
  ...
  if (!(error instanceof ContextplaneApiError)) {
    return {
      description:
        "The administrative request could not be completed. Check the inputs and try again.",
      requestId: null,
  ...
  const byStatus: Readonly<Record<number, { description: string; title: string }>> = {
    ...
    403: {
      description:
        "The current credential does not have the required tenant or deployment-operator authority.",
      title: "Access denied",
    },
    404: {
      description: "The referenced administrative resource no longer exists.",
      title: "Resource not found",
    },
    409: {
      description:
        "The requested change conflicts with current service state. Refresh the relevant resource before retrying.",
      title: "Change conflicts with current state",
    },
    412: {
      description:
        "This resource changed after it was loaded. Preserve the intended change, inspect the latest state, and retry only after review.",
    ...
    429: {
      description: "The service is limiting administrative requests. Wait before trying again.",
      title: "Rate limit reached",
    },
    501: {
      description:
        "This capability is present in the service contract but is not enabled by this deployment.",
      title: "Capability unavailable",
    },
    503: {
      description:
        "The administrative service is temporarily unavailable. No success should be assumed.",
      title: "Service unavailable",
    },
  };
  const failure = byStatus[error.status] ?? {
    description:
      "The service did not complete this administrative request. No success should be assumed.",
    title: "Request failed",
  };
...
export function AdminOperationDialog({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      showToast({
        message: "The service accepted and completed the requested operation.",
  ...
  const canSubmit = !mutation.isPending && (!operation.destructive || confirmation === "CONFIRM");

  return (
    <dialog
      ref={dialogRef}
      aria-describedby="admin-operation-description"
    ...
    >
      <div>
        <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-border bg-surface px-6 py-5">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <StatusBadge tone={operation.scope === "operator" ? "warning" : "info"}>
                {operation.scope === "operator" ? "Deployment operator" : "Tenant administrator"}
              </StatusBadge>
              {operation.destructive ? <StatusBadge tone="danger">Destructive</StatusBadge> : null}
            </div>
            <h2 id="admin-operation-title" className="mt-3 text-lg font-semibold text-foreground">
              {operation.title}
            </h2>
            <p id="admin-operation-description" className="mt-1 text-sm text-muted">
        ...
        <form className="space-y-6 p-6" onSubmit={submit}>
          ...
          {operation.scope === "operator" ? (
            <Notice title="Deployment-operator identity is required" variant="warning">
              The service verifies operator identity for this action. Tenant administrator access
          ...
          {(operation.queryParameters?.length ?? 0) > 0 ? (
            <fieldset>
              <legend className="text-sm font-semibold text-foreground">Optional filters</legend>
              <p className="mt-1 text-sm text-muted">Blank values are omitted from the request.</p>
              <div className="mt-3 grid gap-4 sm:grid-cols-2">
                {operation.queryParameters?.map((parameter) => (
          ...
          {operation.bodyExample !== undefined ? (
            <label className={labelClassName}>
              ...
              <span className="mt-1 block font-normal text-muted">
                JSON matching {operation.requestSchema ?? "the documented request schema"}.
              </span>
              <textarea
                aria-describedby={bodyError ? "admin-body-error" : undefined}
              ...
              />
              {bodyError ? (
                <span id="admin-body-error" className="mt-2 block text-sm font-medium text-danger">
                  {bodyError}
                </span>
              ) : null}
            </label>
              ...
              <div className="flex gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-semibold text-foreground">
                    This action can remove or invalidate governed state.
                  </p>
